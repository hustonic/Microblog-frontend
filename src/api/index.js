...
const api = {
    ...
    relation: {
        ...
        listRelation(data) {
            ...
            param.append('offset', data.offset);
            param.append('limit', data.limit);
            return Vue.prototype.$axios.get(this.prefix + '/relations' + `?${param}`);
        },
        insertRelation(data) {
            return Vue.prototype.$axios.post(this.prefix + '/relations', data.userIdPub, {
                headers: {
                    'Content-Type': 'application/json'
                }
            });
        ...
        deleteRelation(data) {
            return Vue.prototype.$axios.delete(this.prefix + `/relations/${data.relaId}`);
        },
        updateRelation(data) {
            let param = new URLSearchParams();
            param.append('relaRemark', data.relaRemark);
            return Vue.prototype.$axios.patch(this.prefix + `/relations/${data.relaId}` + `?${param}`);
        },
        countRelation(data) {
            let param = new URLSearchParams();
            if (data.userIdSubscriber) param.append('userIdSubscriber', data.userIdSubscriber);
    ...
    verification: {
        ...
        getVerification(data) {
            return Vue.prototype.$axios.get(this.prefix + `/verifications/${data.userId}`)
        },
        addVerification(data){
            return Vue.prototype.$axios.post(this.prefix + '/verifications', data)
        },
        listVerifyingVerification(data){
            let param = new URLSearchParams();
            param.append('offset', data.offset);
            param.append('limit', data.limit);
            return Vue.prototype.$axios.get(this.prefix + '/verifications' +`?${param}`)
        },
        verify(data){
            return Vue.prototype.$axios.patch(this.prefix + '/verifications' ,data)
        },
        confirm(data){
            return Vue.prototype.$axios.patch(this.prefix + `/verifications/${data.verId}`)
        },
        deleteVerification(data){
            return Vue.prototype.$axios.delete(this.prefix + `/verifications/${data.verId}`)
        }
    }
};
